...
App.Utils.CalendarTableView = (function () {
    ...
    function createAppointments($providerColumn, appointments) {
        ...
        for (const index in appointments) {
            ...
            // Añadir indicador visual si la cita tiene notas
            let titleText = title.join(' - ');
    ...
    function getEventNotes(event) {
        ...
        // Add appointment notes if available
        if (data.notes) {
            notesText += data.notes;
        }

        // Add custom_field_1 (required objects) if available and customer data exists
        if (data.customer && data.customer.custom_field_1) {
            if (notesText) {
                notesText += '\n\n';
            }
            notesText += 'Objetos requeridos: ' + data.customer.custom_field_1;
        }

        // Return dash if no notes or custom field
        if (!notesText) {
            return '-';
        }

        return notesText.length > 150 ? notesText.substring(0, 150) + '...' : notesText;
    }
