...
App.Http.Booking = (function () {
    ...
    function registerAppointment() {
        ...
        $.ajax({
            ...
            beforeSend: () => {
                ...
                $spinner.append(
                    $('<div/>').addClass('spinner-border').css({
                        width: '3rem',
                        height: '3rem',
                        'margin-bottom': '1rem',
                    })
                ...
                $spinner.append(
                    $('<div/>').text('Procesando reserva...').css({
                        'font-size': '1.2rem',
                        'font-weight': 'bold',
                    })
                );
                
                // Check if multiple appointments
                const appointments = formData.appointments;
                if (appointments && appointments.length > 1) {
                    $spinner.append(
                        $('<div/>').text(`Creando ${appointments.length} citas...`).css({
                            'font-size': '1rem',
                            'margin-top': '0.5rem',
                        })
                    );
     ...
     * 
     * @param {Number} serviceId The service ID to check
     * @returns {Promise} Promise that resolves with array of dates (YYYY-MM-DD format)
     */
    function getCustomerServiceDates(serviceId) {
        const appointmentId = App.Pages.Booking.manageMode ? vars('appointment_data').id : null;

        const url = App.Utils.Url.siteUrl('booking/get_customer_service_dates');
