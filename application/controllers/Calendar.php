...
class Calendar extends EA_Controller
{
    ...
    {
        // Si el usuario es un cliente (customer), redirigir a booking
    ...
    public function get_calendar_appointments(): void
    {
        try {
            ...
            foreach ($response['appointments'] as &$appointment) {
                ...
                // Adjust full-day appointments to show within working hours
                $this->adjust_full_day_appointment_display($appointment);
            }
    ...
    /**
     * Adjust full-day appointments to display within provider's working hours
     * 
     * @param array &$appointment Appointment data (passed by reference)
     */
    private function adjust_full_day_appointment_display(array &$appointment): void
    {
        try {
            $start_time = date('H:i', strtotime($appointment['start_datetime']));
            $end_time = date('H:i', strtotime($appointment['end_datetime']));
            
            // Check if this is a full-day appointment (00:00:00 - 23:59:59 or 00:00 - 23:59)
            if (($start_time === '00:00' && ($end_time === '23:59' || $end_time === '00:00'))) {
                ...
                log_message('debug', 'Full-day appointment detected for date: ' . $appointment_date . ', day: ' . $day_of_week);
                
                // Get the working plan
                $working_plan = null;
                
                if (!empty($provider['settings']['working_plan'])) {
                    if (is_string($provider['settings']['working_plan'])) {
                        $working_plan = json_decode($provider['settings']['working_plan'], true);
                    } else if (is_array($provider['settings']['working_plan'])) {
                        $working_plan = $provider['settings']['working_plan'];
                    }
                }
                
                log_message('debug', 'Working plan: ' . json_encode($working_plan));
                
                if ($working_plan && isset($working_plan[$day_of_week])) {
                    $day_plan = $working_plan[$day_of_week];
                    
                    log_message('debug', 'Day plan for ' . $day_of_week . ': ' . json_encode($day_plan));
                    
                    // Only adjust if the day is not marked as not working
                    if (!empty($day_plan['start']) && !empty($day_plan['end'])) {
                        // Update the appointment times to reflect working hours
                        $appointment['start_datetime'] = $appointment_date . ' ' . $day_plan['start'] . ':00';
                        $appointment['end_datetime'] = $appointment_date . ' ' . $day_plan['end'] . ':00';
                        
                        log_message('debug', 'Adjusted appointment times: ' . $appointment['start_datetime'] . ' to ' . $appointment['end_datetime']);
                    } else {
                        log_message('debug', 'Day is marked as not working or has empty start/end times');
                    }
                } else {
                    log_message('debug', 'Working plan not found or day not in working plan');
                }
            } else {
                log_message('debug', 'Not a full-day appointment. Start: ' . $start_time . ', End: ' . $end_time);
            }
        } catch (Exception $e) {
            // If there's any error, leave the appointment times as-is
            log_message('error', 'Error adjusting full-day appointment display: ' . $e->getMessage());
        }
    }
}
