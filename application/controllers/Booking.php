...
class Booking extends EA_Controller
{
    ...
    public function index(): void
    {
        ...
        // Verificar autenticación - Solo usuarios registrados pueden acceder al booking
        if (!session('user_id')) {
            // Guardar la URL destino para redirigir después del login
            session(['dest_url' => site_url('booking')]);
            redirect('login');
            return;
        }
        ...
        if (!empty($appointment_hash)) {
            ...
            // Obtener los datos del usuario logueado para autocompletar el formulario
            $user_id = session('user_id');
            if ($user_id) {
                $user = $this->users_model->find($user_id);
                // Buscar si el usuario ya existe como cliente
                $existing_customer = $this->customers_model->query()
                    ->where('id', $user_id)
                    ->get()
                    ->row_array();
                ...
                if (!empty($existing_customer)) {
                    ...
                    // Si no existe como cliente, crear un array con los datos básicos del usuario
                    $customer = [
                        'id' => null,
    ...
    public function register(): void
    {
        try {
            ...
            // Handle both single appointment and multiple appointments
            $appointments = [];
            if (isset($post_data['appointments']) && is_array($post_data['appointments'])) {
                // Multiple appointments (multiple hours selected)
                $appointments = $post_data['appointments'];
            } elseif (isset($post_data['appointment'])) {
                // Single appointment
                $appointments = [$post_data['appointment']];
            } else {
                throw new RuntimeException('No appointment data provided');
            }
            ...
            // Get service info (same for all appointments)
            ...
            // Get provider info (use the provider from the first appointment)
            $provider_id = $first_appointment['id_users_provider'] ?? null;
            ...
            // Si hay un usuario logueado, usar su ID directamente
            $logged_user_id = session('user_id');
            if ($logged_user_id) {
                $customer['id'] = $logged_user_id;
            } elseif ($this->customers_model->exists($customer)) {
                $customer['id'] = $this->customers_model->find_record_id($customer);
            }

            // Validate each appointment for conflicts
            foreach ($appointments as $appointment) {
                if (!empty($customer['id'])) {
                    // Check if already has an appointment at this time
                    ...
                    // For full-day appointments, check if already has the same service that day
                    $start_time = date('H:i:s', strtotime($appointment['start_datetime']));
                    $end_time = date('H:i:s', strtotime($appointment['end_datetime']));
                    $is_full_day = ($start_time === '00:00:00' && $end_time === '23:59:59');
                    
                    if ($is_full_day && !empty($appointment['id_services'])) {
                        ...
                        $same_service_appointments = $this->appointments_model->get([
                            'id !=' => $manage_mode ? $appointment['id'] : null,
                            'id_users_customer' => $customer['id'],
                        ...
                        if (count($same_service_appointments)) {
                            throw new RuntimeException(lang('customer_already_has_service_on_date'));
                        }
                    }
                }
            }
            ...
            // Si el cliente ya existe, preservar los custom_fields que no se envían desde booking
            if (!empty($customer['id'])) {
                $existing_customer = $this->customers_model->find($customer['id']);
                
                // Preservar custom_fields solo si no vienen en el request o están vacíos
                for ($i = 1; $i <= 5; $i++) {
                    $field = 'custom_field_' . $i;
                    if (empty($customer[$field]) && !empty($existing_customer[$field])) {
                        $customer[$field] = $existing_customer[$field];
                    }
                }
                
                // Preservar notas del cliente si no vienen en el request o están vacías
                if (empty($customer['notes']) && !empty($existing_customer['notes'])) {
                    $customer['notes'] = $existing_customer['notes'];
                }
            }
            ...
            foreach ($appointments as $appointment) {
                ...
                // Only calculate end_datetime if it's not a full-day appointment
                $start_time = date('H:i:s', strtotime($appointment['start_datetime']));
                $end_time = date('H:i:s', strtotime($appointment['end_datetime']));
                $is_full_day = ($start_time === '00:00:00' && $end_time === '23:59:59');
                ...
                $saved_appointments[] = $appointment;
                
                if ($first_appointment_hash === null) {
                    $first_appointment_hash = $appointment['hash'];
                }

                // Sync and notify for each appointment
            ...
            $response = [
                'appointment_id' => $saved_appointments[0]['id'],
                'appointment_hash' => $first_appointment_hash,
                'appointments_count' => count($saved_appointments),
            ];
    ...
    protected function check_datetime_availability(): ?int
    {
        ...
        // Check if this is a full-day appointment (00:00:00 - 23:59:59)
        $is_full_day = ($hour === '00:00' && $appointment_end->format('H:i') === '23:59');

        if ($appointment['id_users_provider'] === ANY_PROVIDER) {
            $appointment['id_users_provider'] = $this->search_any_provider($appointment['id_services'], $date, $hour, $is_full_day);
        ...
        // For full-day appointments, skip the available hours check
        if ($is_full_day) {
            // Just verify the provider is working on this date by checking if there are any available hours
            ...
            // If there are any available hours on this date, allow the full-day booking
            return count($available_hours) > 0 ? $appointment['id_users_provider'] : null;
        }
     ...
     * @param bool $is_full_day Whether this is a full-day appointment.
     *
     * @return int|null Returns the ID of the provider that can provide the service at the selected date.
     *
     * @throws Exception
     */
    protected function search_any_provider(int $service_id, string $date, ?string $hour = null, bool $is_full_day = false): ?int
    {
        $available_providers = $this->providers_model->get_available_providers(true);
        ...
        foreach ($available_providers as $provider) {
            foreach ($provider['services'] as $provider_service_id) {
                if ($provider_service_id == $service_id) {
                    ...
                    if ($is_full_day) {
                        // For full-day appointments, just check if provider has any availability on this date
    ...
    public function get_available_hours(): void
    {
        try {
            ...
            // Filtrar las horas que ya están ocupadas por otros usuarios del mismo servicio
            $where = [
                'id_services' => $service_id,
            ];

            // Si estamos en modo edición, excluir la cita actual
            if ($exclude_appointment_id) {
                $where['id !='] = $exclude_appointment_id;
            }

            $appointments = $this->appointments_model->get($where);
            ...
            $response = array_filter($response, function($hour) use ($occupied_hours) {
                return !in_array($hour, $occupied_hours);
            });
    ...
    /**
     * Get dates where customer already has appointments for a specific service.
     * 
     * This endpoint returns all dates where the logged-in customer already has
     * a booking for the selected service. Used to prevent duplicate bookings.
     */
    public function get_customer_service_dates(): void
    {
        try {
            $disable_booking = setting('disable_booking');
            ...
            // Si no se especifica servicio, devolver array vacío
            if (empty($service_id)) {
                json_response([]);
                return;
            }

            // Obtener TODAS las citas para este servicio (de cualquier usuario)
            $where = [
                'id_services' => $service_id,
            ];

            // Si estamos en modo edición, excluir la cita actual
            if ($manage_mode && $appointment_id) {
                $where['id !='] = $appointment_id;
            }

            $appointments = $this->appointments_model->get($where);

            // Extraer solo las fechas (sin hora) donde hay al menos una cita
            $booked_dates = [];
            foreach ($appointments as $appointment) {
    ...
    /**
     * Get occupied hours for a specific service and date.
     * Returns an array of hours (in HH:mm format) that are already booked by any user.
     */
    public function get_occupied_hours(): void
    {
        try {
            $disable_booking = setting('disable_booking');
            ...
            // Si no se especifican parámetros requeridos, devolver array vacío
            if (empty($service_id) || empty($selected_date)) {
                json_response([]);
                return;
            }

            // Obtener todas las citas para este servicio en la fecha seleccionada
            $where = [
                'id_services' => $service_id,
            ];
            ...
            // Filtrar solo las citas de la fecha seleccionada y extraer las horas
            $occupied_hours = [];
            foreach ($appointments as $appointment) {
                $appointment_date = date('Y-m-d', strtotime($appointment['start_datetime']));
